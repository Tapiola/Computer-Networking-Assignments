...
class DVRouter (basics.DVRouterBase):
	...
	def handle_link_down (self, port):
		...
		for host in self.table.keys():
			...
			if p == port:
				if self.POISON_MODE:
					self.table[host] = INFINITY, p, t
					if host in self.hosts and self.hosts[host][0] < self.table[host][0]:
	...
	def handle_rx (self, packet, port):
		...
		if isinstance(packet, basics.RoutePacket):
			...
			#print (self, host_n, self.table[host_n])
			...
			if not packet.dst in self.table or self.table[packet.dst][0] >= INFINITY:
				return
	...
	def handle_timer (self):
		...
		for host in self.table.keys():
			...
			#print ('halp', self, host, self.table[host])
			if not time == 0 and api.current_time() - time > 15:
				#print (api.current_time() - time)
				if self.POISON_MODE:
					self.table[host] = INFINITY, port, -1
					if host in self.hosts and self.hosts[host][0] < self.table[host][0]:
						self.table[host] = self.hosts[host]
				else:
					self.table.pop (host)
					continue
			#print ('---', self, host, self.table[host])
			new_packet = basics.RoutePacket (host, self.table[host][0])
			self.send (new_packet, self.table[host][1], flood=True)
